...
\item
The {\em condition number} of a square matrix~$A$ is 
...
solves $Ax=b$, where $A\succ 0$ is a symmetric, positive definite
...
\item The condition number $\kappa(B^{-1/2}AB^{-1/2}) = \kappa(AB^{-1})$ 
...
the {\em finite condition number} is 
$\kappa_f(A)=\lambda_n/\lambda_k$, where $\lambda_k$ is
the smallest nonzero eigenvalue.
For example, if~$L$ is the Laplacian of a connected graph,
$\kappa_f(L)=\lambda_n/\lambda_2$ is the relevant condition number
for the convergence of conjugate gradient.
...
which is the relevant condition number for the convergence of conjugate
gradient on~$Ax=b$ with preconditioner~$B$.
Note that $\kappa_f(A,B) = \kappa_f(B,A)$.

\item
Let~$A$ and~$B$ be two symmetric positive semidefinite matrices
with the same null space.
If $\alpha B \preceq A \preceq \beta B$,
